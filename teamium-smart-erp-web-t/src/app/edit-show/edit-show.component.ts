...
export class EditShowComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //this.findProjectByBudgetApi = this.httpService.callApi('getProjectByBudget', { pathVariable: '/' + this.showId });
    this.editLineForm = this.creatEditLineFormGroup();

    forkJoin([this.dropdownDataAPI, this.projectDetailByIdAPI]).subscribe(resultList => {
      ...
      this.resourceTypes = ["Equipment", "Personnel", "Services", "Copyright", "Supplier"];
  ...
  getResource(resourceType): any {
    ...
    console.log(this.dropdownData.functionsByType, resourceTypeToGet)
  ...
  showAddLineModalView() {
    if (this.projectDetail.financialStatus == 'Rejected') {
      this.toastr.warning("Cannot add lines as the show has been rejected.");
    }
    else if (this.projectDetail.financialStatus == 'Approved') {
      this.toastr.warning("Cannot add lines as the show has been approved.");
    } else {
  ...
  showLineEditModal(line: any) {
    ...
    if (this.projectDetail.financialStatus == 'Rejected') {
      this.toastr.warning("Cannot edit line as the show has been rejected.");
    }
    else if (this.projectDetail.financialStatus == 'Approved') {
      this.toastr.warning("Cannot edit line as the show has been approved.");
    } else {
      ...
      this.editLineForm.get('discountRate').setValue(this.selectedLine.discountRate);
      this.editLineForm.get('discountAmount').setValue(this.selectedLine.discountAmount);
      this.editLineForm.get('vatRate').setValue(this.selectedLine.vatRate);
  ...
  addLine(resourceFunction: any, index) { 
    ...
    let newLine = {
      ...
      "start": moment(to).tz("Asia/Kolkata").format(),
      "end": moment(from).tz("Asia/Kolkata").format(),
    }
  ...
  removeLine(line: any) {
    if(confirm("Are you sure to delete this line?")){
      this.httpService.callApi('deleteLineFromShow', {  pathVariable: line.id}).subscribe(response => {
        this.blockedPanel = false;
  ...
  addOrSaveLine(line: any, message: string) {
    this.blockedPanel = true;
    this.httpService.callApi('addLineToShow', { pathVariable: this.projectDetail.id, body: line }).subscribe(response => {
      this.blockedPanel = false;
      this.projectDetail = response;
      this.toastr.success(message, "Show")
      this.isEditLineModalOpen=false;
    }, error => {
      this.blockedPanel = false;
      this.toastr.error(error.error.message)
  ...
  createAllSessions() {
    if(this.projectDetail.sessioned){
      this.toastr.warning("Session already created.");
    }
    else if (this.projectDetail.financialStatus == 'Rejected') {
      this.toastr.warning("Cannot create sessions as the show has been rejected.");
    }
    else if (this.projectDetail.financialStatus == null || this.projectDetail.financialStatus != 'Approved') {
      this.toastr.warning("Cannot create sessions as the show has not been approved yet.");
    } else {
      if (this.projectDetail.lines.length < 1) {
        this.toastr.warning("Please add atleast one line.");
      }
      else {
        if (confirm('Are you sure to create sessions.Once you created, you can not change the lines.Show will freezed.')) {
          this.blockedPanel = true;
          this.httpService.callApi('createAllSessions', {pathVariable: this.projectDetail.id }).subscribe((response) => {
            this.blockedPanel = false;
            this.projectDetail = response;
      
            this.toastr.success("Projects successfully created");
          }, error => {
            this.toastr.error(error.error.message, 'Show');
            console.log('Error getstatus => ', error)
            this.blockedPanel = false;
  ...
  createSession() {
    if(this.projectDetail.sessioned){
      ...
      if (this.projectDetail.lines.length < 1) {
        ...
        if (confirm('Are you sure to create sessions.Once you created, you can not change the lines.Show will freezed.')) {
          this.blockedPanel = true;
          this.httpService.callApi('createSession', { pathVariable: this.projectDetail.id }).subscribe((response) => {
            this.blockedPanel = false;
            this.projectDetail = response;
  ...
  applyBookingDateToAllLines() {
    if (confirm("Are you sure to apply these dates to all bookings?It will change the budget.")) {
      ...
      this.blockedPanel = true;
      this.httpService.callApi('applyDateToAllLinesOnShow', { body: this.selectedLine, pathVariable: this.projectDetail.id }).subscribe((response) => {
        this.blockedPanel = false;
        this.projectDetail = response;
        this.isEditLineModalOpen=false;
        this.selectedLine=null;
