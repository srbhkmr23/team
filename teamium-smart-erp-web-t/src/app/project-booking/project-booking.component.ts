...
@Component({
  ...
  styleUrls: ['./project-booking.component.scss']
})
export class ProjectBookingComponent implements OnInit {
  ...
  showAddAttachmentsLine:boolean;
  ...
  ngOnInit() {
    ...
    let getAvailableGroupsAPI = this.httpService.callApi('getGroups', {})
    this.editLineForm = this.creatEditLineFormGroup();

    forkJoin([this.dropdownDataAPI, findBudgetIdFromProjectIdApi, this.bookingDetailByBudgetIdAPI,getAvailableGroupsAPI]).subscribe(resultList => {
      ...
      this.availableGroups =resultList[3];
  ...
  removeLine(line: any) {
    if (confirm("Are you sure to delete this line?")) {
      this.httpService.callApi('deleteLineFromBokking', { pathVariable: line.id }).subscribe(response => {
        this.blockedPanel = false;
        this.projectDetail = response;
  ...
  goToSchedule(line) {
    ...
    this.dataService.addPathvariables(pathVariable);
    if(line.fromVendor)
    this.dataService.openSubmenu('/teamium/project-procurement/'+this.projectId);
    else
  ...
  addGroupToBooking(line: any, group: any) {
    this.blockedPanel = true;
    let pathVariable =line.id+"/"+group.id;
   
      this.httpService.callApi('addGroupOnBooking', { pathVariable: pathVariable }).subscribe((resp) => {
        line =resp;
        this.blockedPanel = false;
        this.ngOnInit();
      }, (error) => {
  ...
  removeGroupToBooking(line: any) {
    this.blockedPanel = true;
    let pathVariable =line.id//+"/"+group.id;
   
      this.httpService.callApi('removeGroupOnBooking', { pathVariable: pathVariable }).subscribe((resp) => {
        line =resp;
        this.blockedPanel = false;
        this.ngOnInit();
      }, (error) => {
  ...
  onBookingOrderSelect(order:any,line){
    if(order){
  ...
  addAttachmentToLine(line,orderType){
     this.selectedLine=line;
      this.selectedOrder={"orderType":orderType};
      this.showOrderForm=true;
  }

  closeOrderFormModal(){
    this.showOrderForm=false;
    this.bookingDetailByBudgetIdAPI.subscribe((response)=>{
      this.projectDetail=response;
    },(error)=>{
      console.log(error.error.message);
    });
  ...
  onClickedOutside(targetElement) {
    if (!targetElement.closest('#orderAttachment')) {
      this.showAddAttachmentsLine = false;
  ...
  isShowAddAttachmentsLine(lineIndex){
    this.showAddAttachmentsLine=lineIndex;
