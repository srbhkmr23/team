...
@Component({
  selector: 'app-group-scheduler',
  templateUrl: './group-scheduler.component.html',
  styleUrls: ['./group-scheduler.component.scss']
})
export class GroupSchedulerComponent implements OnInit, OnDestroy {
          ...
          args.menu.items = [
            ...
            },
            {
              ...
              onClick: args => {
                var e = args.source;
                let functions = this.functionDropDown.find(element => element.function.id == args.source.data.functionId);
                this.defaultId = functions.defaultResource.id;
                let params = {
                  id: e.data.id,
                  text: e.data.text,
                  start: e.data.start,
      ...
      items: [
        {
          ...
          onClick: args => {
            ...
            let res = this.config.resources.find(element => element.id == args.source.resource);
            console.log("res=> ", res)
  ...
  ngOnInit() {
    ...
    this.dataService.dropDownValueChange.subscribe(value => {
      ...
      let functionValue = this.timeLineForm.get('groupName').value;

      if (value && value.name) {
        if (!functionValue || functionValue.id != value.id) {
          this.timeLineForm.get('groupName').setValue(value);
        }
      } else {
        this.timeLineForm.get('groupName').setValue(null);
    ...
    let groupListApi = this.httpService.callApi('getGroups', {});
    ...
    forkJoin([functionListApi, projectListApi, this.linkEventIdApi, groupListApi]).subscribe((responseList) => {
      if (responseList[0]) {
      ...
      if (responseList[3]) {
        this.groupDropDown = responseList[3];
        let groupName;
        if (this.groupDropDown.length > 0) {
          groupName = this.groupDropDown[0];
          this.timeLineForm.get('groupName').setValue(groupName);
        }
      }
      this.blockedPanel = false;
    }, (error) => {
    ...
    this.timeLineForm.get('groupName').valueChanges.subscribe((value) => {
      if (value) {
        this.blockedPanel = true;
        this.cdr.detectChanges();
        this.multipleSelect = [];
        ...
        if (value.groupMembers) {
          value.groupMembers.forEach(element => {
            res.push({ "id": element.resource.id, "name": element.firstName, "function": element.mainFunction.id })
          });
        }

        this.config.resources = res;

        this.httpService.callApi('getAllEventsByGroup', { pathVariable: "/" + value.id }).subscribe((responce) => {
  ...
  saveOrUpdateEvent(params, cb?: any) {
    ...
    this.httpService.callApi('saveOrUpdateEvent', { body: params }).subscribe((resp) => {
      let eventApi = this.httpService.callApi('getAllEventsByGroup', { pathVariable: "/" + this.timeLineForm.get('groupName').value.id });
  ...
  saveBookingEvents(params) {
    ...
    this.httpService.callApi('saveBookingEvents', { body: params }).subscribe((renponse) => {
      let apiList = [this.httpService.callApi('getAllEventsByGroup', { pathVariable: "/" + this.timeLineForm.get('groupName').value.id }), this.linkEventIdApi];
  ...
  saveCopyEvent(params) {
    ...
    this.httpService.callApi('copyEvent', { body: params }).subscribe((renponse) => {
      let apiList = [this.httpService.callApi('getAllEventsByGroup', { pathVariable: "/" + this.timeLineForm.get('groupName').value.id })];
      forkJoin(apiList).subscribe((responseList) => {
        if (responseList[0]) {
          this.setEvents(responseList[0], renponse.id);
        }
  ...
  unscheduleTask(params: any): void {
    this.httpService.callApi('saveOrUpdateEvent', { body: [params] }).subscribe((renponse) => {
      renponse.forEach(res => {
        this.httpService.callApi('getAllEventsByGroup', { pathVariable: "/" + this.timeLineForm.get('groupName').value.id }).subscribe((responce) => {
          this.events = responce.filter(i => i.resource != this.defaultId);
          this.events.forEach(element => {
            element.start = new DayPilot.Date(moment(element.startTime).tz("Asia/Calcutta").format("YYYY-MM-DD HH:mm:ss"));
            element.end = new DayPilot.Date(moment(element.endTime).tz("Asia/Calcutta").format("YYYY-MM-DD HH:mm:ss"));
  ...
  splitEventApi(params, cb) {
    ...
    this.httpService.callApi('splitEvent', { body: params }).subscribe((resp) => {
      this.httpService.callApi('getAllEventsByGroup', { pathVariable: "/" + this.timeLineForm.get('groupName').value.id }).subscribe((response) => {
